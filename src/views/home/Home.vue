...
export default {
  ...
  mounted() {},
  activated() {  //当组件处于活跃的时候刷新页面并快速滚动到保存的位置
  ...
  methods: {
    // 下面是事件监听的相关方法
    loadMore() {  //加载更多数据，调用加载函数
      this.getHomeGoods(this.curType);
    },
    tabClick(index) {  //当tabbar发送了点击自定义事件过来之后，拿到index
      switch (index) {  //根据index 显示对应的商品数据类型
        case 0:
          this.curType = "pop";
          break;
        case 1:
    ...
    backClick() {  //点击了返回顶部之后，返回顶部
      this.$refs.scroll.scrollTo(0, 0);
    },
    contentScroll(position) {  //当屏幕发生了滚动
      // 1 判断BackTop是否显示↓
      this.isShowBackTop = -position.y > 1000;
      // 2 决定tabControl是否吸顶（position:fix）↓
      this.isTabFixed = -position.y > this.tabOffsetTop;
    },
    swiperImageLoad() {  //轮播图图片加载完成之后取到tabbar的顶部高度
      this.tabOffsetTop = this.$refs.tabControl2.$el.offsetTop;
    },
    // 下面都是网络请求相关的方法
    getHomeData() {
      getHomeData().then(res => { //当数据请求成功的时候
        // 把请求过来的数据存进data里面↓
